...
\textbf{Orbital stability via Floquet theory.} For the full coupled system, one can:
\begin{enumerate}
    \item Find a periodic orbit $\mathbf{x}^*(t)$ numerically.
    \item Linearize: $\dot{\mathbf{y}} = J(t)\,\mathbf{y}$, where $J(t)$ is the periodic Jacobian.
    \item Compute the Floquet multipliers $\mu_i$. Orbital stability holds if $|\mu_i| < 1$ for all $i$ except one trivial $\mu = 1$ (tangential perturbation).
\end{enumerate}
For the coupled network, diagonalization of the graph Laplacian separates the analysis into Fourier modes. The $(1-2u)$ term can induce transverse instabilities when $u > 0.5$, which is precisely the mechanism that \emph{prevents} synchronization and promotes diversity.
...
This mechanism is analogous to \textbf{frustrated spin glasses}: heretics introduce a local inverted field, frustrating ferromagnetic alignment. In random-field Ising models, the transition from ordered to frustrated phase occurs when the disorder concentration exceeds a critical threshold that depends on dimension and coordination number. For a Bethe lattice of coordination $k$, the critical influence threshold is $\eta_c = 1/(k-1)$, giving $\eta_c \approx 0.33$ for $k=4$. In Mem4ristor, the dynamic coupling via the doubt variable $(1-2u)$ effectively amplifies the heretic influence, reducing the critical threshold to approximately $0.15$---consistent with numerical observations across Grid, Small-World, Random, and Scale-Free topologies.
